...
public class CarsListTest {
    ...
    @Test void carSortOrderByLicensePlateNumber() {
        // Using the sort order as you have specified in the constructor of TrafficTracker; fix that first
        Comparator<? super Car> sortOrder = cars.getSortOrder();
        assertEquals(0, sortOrder.compare(scoda, scoda),
                "TrafficTracker should sort its cars by license plate number");
        Car scoda2 = new Car(scoda.getLicensePlate());
        assertEquals(0, sortOrder.compare(scoda, scoda2),
                "TrafficTracker should sort its cars by license plate number only");
    ...
    public void excludeDuplicatesByMerge() {
        ...
        assertEquals(initialCars.size(), cars.nSorted,
                "merging should not change the sort order of items in the list");
        assertSame(volvo2, cars.get(cars.size()-1),
                "merge should add a new car to the unsorted section at the end");
    ...
    public static <E> void testSearchForAll(List<E> list, Function<E, Integer> searcher) {
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.getClass().isInstance(new OrderedArrayList<E>())) {
                ...
                if (index != i && index >= 0 && ((OrderedArrayList<E>) list).getSortOrder().compare(item, list.get(index)) == 0) {
    ...
    public static <E> void checkRepresentationInvariant(OrderedList<E> list) {
        ...
        for (int i = 0; i < orderedArrayList.nSorted-1; i++) {
            assertThat(
                    ...
                    orderedArrayList.getSortOrder().compare(orderedArrayList.get(i), orderedArrayList.get(i+1)), lessThanOrEqualTo(0));
        }
    }
}
